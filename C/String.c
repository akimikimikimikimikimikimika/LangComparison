...
void String(){
	...
	n();printf("これから文字列操作を試します");n();

	// 文字列の長さを測定
	integer = strlen("someString");
	printf("strlen(\"someString\") = %d",integer);n();

	// str1の文字列の内容をstr2にコピー
	strcpy(str2,str1);
	printf("strcpy: \"%s\" → \"%s\"",str1,str2);n();

	// str1の文字列の内容をstr3の後に付け加える
	strcat(str3,str1); // str3=str3+str1
	printf("strcat: \"%s\" → \"%s\"",str1,str3);n();

	// str1とstr3の内容を比較
	integer=strcmp(str1,str3);
	printf("strcmp: %d",integer);n();

	// str3の中から文字列を検索
	strptr=strstr(str3,"some");
	printf("strstr: %s",strptr);n();
