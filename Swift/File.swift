...
io.write("\r\n## Header 2\r\n### Header 3".data(using: .utf8)!)
	/*
		...
		勿論だが, io.seekToEndOfFile() を省けば先頭から書き込むことになる。しかし,この場合,新たに書き込む内容が既にファイルに書き込まれているデータよりも少ない場合,前のデータが残ってしまうことがある。それを防ぐには,前に紹介した fm.createFile() を使った方が手っ取り早い。この場合,ファイルが既に存在していても上書きされるだけである。
	*/
io.closeFile()
